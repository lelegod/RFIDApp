...
public class ConnectDeviceActivity extends Activity implements View.OnClickListener,NotifyForActivityInterface, RadioGroup.OnCheckedChangeListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mConnectedString = getIntent().getStringExtra(MainMenuActivity.KEY_CONNECTED);
        ...
        mConnectedDeviceName = getIntent().getStringExtra(MainMenuActivity.KEY_DEVICENAME);
        ...
        // 未Open時はOpenする
        if (MainMenuActivity.getSDKLibrary().getState() == TecRfidSuite.OPOS_S_CLOSED) {
            int nRet = TecRfidSuite.OPOS_SUCCESS;
            if (((RadioButton) findViewById(R.id.radioButtonUF3000)).isChecked() == true) {
                nRet = MainMenuActivity.getSDKLibrary().open("UF-3000", MainMenuActivity.getInstance().getMainMenuActivityContext());
                if (TecRfidSuite.OPOS_SUCCESS != nRet) {
                    // 不具合以外ではここにこない
                    return;
                }
                mConnectedDeviceName = "UF-3000";
                MainMenuActivity.getSDKLibrary().startBluetoothDiscovery(MainMenuActivity.getInstance().getMainMenuActivityContext(), mBluetoothDiscoveryEvent);
            }
            else {
                nRet = MainMenuActivity.getSDKLibrary().open("UF-2200", MainMenuActivity.getInstance().getMainMenuActivityContext());
                if (TecRfidSuite.OPOS_SUCCESS != nRet) {
                    // 不具合以外ではここにこない
                    return;
                }
        ...
        if (TecRfidSuite.OPOS_SUCCESS == MainMenuActivity.getSDKLibrary().getBluetoothList(mAdressArray)) {
            // デバイスリストを更新
    ...
    protected void onResume() {
        ...
        if (MainMenuActivity.getSDKLibrary().getState() == TecRfidSuite.OPOS_S_CLOSED) {
            if(mConnectedDeviceName.equals("UF-3000")){
                //UF-3000
                MainMenuActivity.getSDKLibrary().open("UF-3000",MainMenuActivity.getInstance().getMainMenuActivityContext());
            } else {
                //UF-2200
                MainMenuActivity.getSDKLibrary().open("UF-2200",MainMenuActivity.getInstance().getMainMenuActivityContext());
            }
        }
        if (TecRfidSuite.OPOS_SUCCESS != MainMenuActivity.getSDKLibrary().startBluetoothDiscovery(this, mBluetoothDiscoveryEvent)){
    ...
    protected void onPause() {
        ...
        if (TecRfidSuite.OPOS_SUCCESS != MainMenuActivity.getSDKLibrary().stopBluetoothDiscovery()){
    ...
    public boolean onMenuItemSelected(int featureId, MenuItem item) {
        switch (item.getItemId()) {
            ...
            default:
                ...
                //ラジオボタン変更の有無をMainMenuActivityに渡す
                data.putExtra(MainMenuActivity.KEY_CHANGEDEVICERADIOBUTTON, mChangeDeviceRadioButton);
                // 結果を設定
    ...
    public void onClick(View view) {
        ...
        if (view.equals(mConnectionBtn)) {
            ...
            if (mCheckIndex != -1) {
                ...
                Intent data = new Intent();
                // インテントに値をセット
                data.putExtra(MainMenuActivity.KEY_CONNECTIONREQUEST, checkText.getText().toString());
                //選択されたデバイス名をMainMenuActivityに渡す
                data.putExtra(MainMenuActivity.KEY_DEVICENAME, mConnectedDeviceName);
                //ラジオボタン変更の有無をMainMenuActivityに渡す
                data.putExtra(MainMenuActivity.KEY_CHANGEDEVICERADIOBUTTON, mChangeDeviceRadioButton);
                // 結果を設定
    ...
    public void onCheckedChanged(RadioGroup group, int checkedId) {
        ...
        //BLEデバイス検索終了
        MainMenuActivity.getSDKLibrary().stopBluetoothDiscovery();
        ...
        if (MainMenuActivity.getSDKLibrary().getState() != TecRfidSuite.OPOS_S_CLOSED) {
            ...
            //クローズしてしまう
            MainMenuActivity.getSDKLibrary().close();
        }
